...
class BooksSpider(CrawlSpider):
    ...
    def parse_detail_page(self, response):
        ...
        item['price'] = self.parse_double(price)
        item['normal_price'] = self.parse_double(normal_price)
        item['discount'] = self.parse_int(discount)
        item['original_name'] = original_name
        item['page_number'] = self.parse_int(page_number)
        item['pledge'] = pledge
        item['size'] = size
        item['weight'] = self.parse_int(weight)
        item['language'] = language
        item['isbn'] = isbn
        item['release_year'] = self.parse_int(release_year)
        item['publishing_house'] = publishing_house
        item['cat_number'] = self.parse_int(cat_number)
    ...
    def parse_double(self, string_double):
        if string_double is None:
            return string_double

        return float(string_double)

    def parse_int(self, string_int):
        if string_int is None:
            return string_int
